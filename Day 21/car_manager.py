...
CARS_START_AT_X_PERCENTAGE = 0.8

class CarManager(Turtle):
    def __init__(self, screen_size, cars_colors, initial_vel, vel_increment):
        super().__init__()
    ...
    def initialize_cars(self):
        ...
        for i in range(0, NUMBER_OF_LANES):
            ...
            self.cars_speeds_x.append(random.randint(int(self.vel / 2), self.vel))
    ...
    def check_if_wave_is_finished(self):
        ...
        for car in self.cars:
            if car.position()[0] < 0:
                print('DEBUG cars position', car.position())
                return True
        return False
